...
- **The in-page links between the two screens are hidden under Cockpit.** Cockpit's own
  tools menu already lists both, so the pair inside the page was a second way to do the
  same thing — which only leaves the reader deciding which one to use. Standalone has no
  shell, so there they stay: that is the only way across.
