...
なお `scrub status` だけは非特権で動きます（`/var/lib/btrfs/` の記録を読むため）。
`balance status` は他と同じく root が必要です。
...
3. **ダッシュボードとデバイス管理** — `devices`・`scrub`・`balance`・
   ダッシュボード画面は実装済み。次はデバイスの追加・取り外し・置換
4. **デスクトップ GUI**
